...
def test_model(model, threshold, Xnew, ynew, uses_proba=True):
    ...
    metrics = {
        ...
        "f1.25": fbeta_score(ynew, ypred, beta=1.25, zero_division=np.nan),
        "f1.5": fbeta_score(ynew, ypred, beta=1.5, zero_division=np.nan),
        "f1.75": fbeta_score(ynew, ypred, beta=1.75, zero_division=np.nan),
